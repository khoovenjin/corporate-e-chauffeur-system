...
function AnalysisPieChart({ viewMode }) {
    ...
    function processTripDataToDisplay() {
        ...
        const firstProcessedArray = unique_vehicle_options.map((element, index)=>{
            ...
            return {
                id: index+1,
                name: element,
                y: (viewMode == 0) ? result.length : moneySpent.toFixed(2),
                label: (viewMode == 0) ? `${parseFloat(result.length / sampleTrips.length * 100).toFixed(2)}%` : `${(moneySpent / totalMoneySpent * 100).toFixed(2)}%`,
                // moneySpent: moneySpent.toFixed(2),
                // moneySpentPercentage: `${(moneySpent / totalMoneySpent * 100).toFixed(2)}%`,
            }
        });
        ...
        const midProcessedArray = firstProcessedArray.sort((a,b)=>{
            ...
            return {
                ...item,
                colorRide: chartColorPallet[index]
            }
        });
        ...
        const finalProcessedArray = midProcessedArray.sort((a,b)=>{
            ...
            return {
                ...item,
                colorMoney: chartColorPallet[index]
            }
    ...
    function renderChart() {
        ...
        let colorScales = chartData.map((item) => {
            return (viewMode == 0) ? item.colorRide : item.colorMoney
        });
    ...
    function renderVehicleSummary() {
        ...
        const renderItem = ({ item }) => (
            <TouchableOpacity
                style={{
                    ...styles.rideListItem,
                    backgroundColor: (selectedVehicle && selectedVehicle.name == item.name) ? item.colorRide : defaultStyles.colors.white
                }}
                onPress={() => {
                ...
                <View style={styles.summaryContainer}>
                    <View
                        style={{
                            ...
                            backgroundColor: (selectedVehicle && selectedVehicle.name == item.name) ? defaultStyles.colors.white : item.colorRide,
                        }}
                    />
                    <AppText style={{
                        ...styles.summaryText,
