...
import { sampleCareerWithdrawals, sampleMonthWithdrawals, sampleTrips } from "../../config/data";
...
function DriverAnalysisPieChart({ viewMode }) {
    ...
    function processWithdrawalsToDisplay() {
        const firstProcessedArray = () => {
            ...
            const careerWithdrawals = sampleCareerWithdrawals.reduce((acc, item) => acc + (item.Withdrawal_Amount || 0), 0);
            const totalEarnings = sampleTrips.reduce((acc, item) => acc + (item.price || 0), 0);
            const monthReceivable = totalEarnings - monthWithdrawals;
            const careerReceivable = totalEarnings - careerWithdrawals;

            return [
                {
                    id: 1,
                    name: 'Withdrawals',
                    y: (viewMode == 0) ? parseFloat(monthWithdrawals.toFixed(2)) : parseFloat(careerWithdrawals.toFixed(2)),
                    label: (viewMode == 0) ? `${(monthWithdrawals / totalEarnings * 100).toFixed(2)}%` : `${(careerWithdrawals / totalEarnings * 100).toFixed(2)}%`,
                    color: chartColorPallet[0]
                },
                {
                    id: 2,
                    name: 'Receivables',
                    y: (viewMode == 0) ? parseFloat(monthReceivable.toFixed(2)) : parseFloat(careerReceivable.toFixed(2)),
                    label: (viewMode == 0) ? `${(monthReceivable / totalEarnings * 100).toFixed(2)}%` : `${(careerReceivable / totalEarnings * 100).toFixed(2)}%`,
        ...
        const renderItem = ({ item }) => (
            <TouchableOpacity
                style={{
                    ...
                    backgroundColor: (selectedMode && selectedMode.name == item.name) ? item.color : defaultStyles.colors.white
                }}
                onPress={() => {
...
export default DriverAnalysisPieChart;