...
function CorporatePreferencePieChart({ viewMode }) {
    ...
    function processTripDataToDisplay() {
        ...
        const firstProcessedArray = unique_vehicle_options.map((element, index)=>{
            ...
            // sampleTrips is the dataset got from DynamoDB filter by month/overall
            ...
            return {
                id: index+1,
                name: element,
                y: (viewMode == 0) ? result.length : result.length,
                label: (viewMode == 0) ? `${parseFloat(result.length / sampleTrips.length * 100).toFixed(2)}%` : `${parseFloat(result.length / sampleTrips.length * 100).toFixed(2)}%`,
            }
        });
        ...
        const midProcessedArray = firstProcessedArray.sort((a,b)=>{
            ...
            return {
                ...item,
                colorRideMonth: chartColorPallet[index]
            }
        });
        ...
        const finalProcessedArray = midProcessedArray.sort((a,b)=>{
            ...
            return {
                ...item,
                colorRideOverall: chartColorPallet[index]
            }
    ...
    function renderChart() {
        ...
        let colorScales = chartData.map((item) => {
            return (viewMode == 0) ? item.colorRideMonth : item.colorRideOverall
        });
            ...
            return (
                <View style={styles.chartContainer}>
                    ...
                    <View style={styles.pieSummary}>
                        ...
                        <AppText style={{ ...styles.subTitle, textAlign: 'center' }}>Rides</AppText>
                    </View>
                </View>
            )
        }
    ...
    function renderVehicleSummary() {
        ...
        const renderItem = ({ item }) => (
            <TouchableOpacity
                style={{
                    ...styles.rideListItem,
                    backgroundColor: (selectedVehicle && selectedVehicle.name == item.name) ? item.colorRideMonth : defaultStyles.colors.white
                }}
                onPress={() => {
            ...
            >
                ...
                <View style={styles.summaryContainer}>
                    <View
                        style={{
                            ...
                            backgroundColor: (selectedVehicle && selectedVehicle.name == item.name) ? defaultStyles.colors.white : item.colorRideMonth,
                        }}
                    />
                    <AppText style={{
                        ...styles.summaryText,
                ...
                <View style={{ justifyContent: 'center' }}>
                    <AppText style={{
                        ...
                        fontSize: 14,
                        fontWeight: 'bold'
                    }}>{item.y} Rides - {item.label}</AppText>
                </View>
            </TouchableOpacity>
        )
...
export default CorporatePreferencePieChart;