...
const passenger = {
    name: passengerList[0].name,
    phone: passengerList[0].phone,
    email: passengerList[0].email,
    image: passengerList[0].image
}
...
function CorporateSettingScreen({ navigation }) {
    const {user} = useAuth();
    ...
    const successAlert = () => {
        Alert.alert('Personal Information Updated!', 'Your personal information has been updated successfully.',
            [{ text: 'Okay', onPress: ()=>navigation.navigate(routes.CORPORATE_ACCOUNT)}]
        )
    }
    ...
    const handleSubmit = async ({ name, phone, email }, { resetForm }) => {
        const result = await corporateApi.getCorporateByEmail(email);
        ...
        const updateResult = await corporateApi.updateCorporateSettingProfile(user.userId.toString(),{
            name: name,
            phone: phone,
            email: email
        });
...
export default CorporateSettingScreen;